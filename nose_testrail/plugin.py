...
class NoseTestRail(Plugin):
    ...
    def send_result(self, result):
        if self.test_case_id:
            ...
            if host:
                uri = 'https://{0}/index.php?/api/v2/add_result_for_case/{1}/'.format(
                        host, run_id)
                if type(self.test_case_id) is str:
                    for test_case_id in self.test_case_id.strip("[,]").split(","):
                        uri = uri + test_case_id
                        self.__send_request('POST', uri, result)
                else:
                    uri = uri + str(self.test_case_id)
                    self.__send_request('POST', uri, result)         
